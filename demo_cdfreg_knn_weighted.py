...
from npsurvival_models import CDFRegressionKNNWeightedSurvival
...
survival_estimator_name = 'cdfreg_knn_weighted'  # for this file
...
kernel_desc_pairs = \
    [(kernel_box, 'cdfreg k-NN'),
     (kernel_triangle, 'cdfreg k-NN (triangle)'),
     (kernel_epanechnikov, 'cdfreg k-NN (Epanechnikov)'),
     (kernel_trunc_gauss1, 'cdfreg k-NN (truncated Gauss sigma=1)'),
     (kernel_trunc_gauss2, 'cdfreg k-NN (truncated Gauss sigma=2)'),
     (kernel_trunc_gauss3, 'cdfreg k-NN (truncated Gauss sigma=3)')]
        ...
        for (kernel_func, desc), metric \
            ...
            for k in hyperparams:
                ...
                for train_idx, val_idx in kf.split(X_train):
                    ...
                    surv_model = \
                        CDFRegressionKNNWeightedSurvival(n_neighbors=k,
                                                         n_jobs=-1,
                                                         metric=metric)
            ...
            for k in best_hyperparams:
                ...
                surv_model = CDFRegressionKNNWeightedSurvival(n_neighbors=k,
                                                              n_jobs=-1,
                                                              metric=metric)
