...
from npsurvival_models import KernelSurvival
...
survival_estimator_name = 'kernel'  # for this file
...
kernel_desc_pairs = \
    [(kernel_box, 'kernel (box)'),
     (kernel_triangle, 'kernel (triangle)'),
     (kernel_epanechnikov, 'kernel (Epanechnikov)'),
     (kernel_trunc_gauss1, 'kernel (truncated Gauss sigma=1)'),
     (kernel_trunc_gauss2, 'kernel (truncated Gauss sigma=2)'),
     (kernel_trunc_gauss3, 'kernel (truncated Gauss sigma=3)')]
...
for experiment_idx in range(n_experiment_repeats):
    for dataset in datasets:
        ...
        hyperparams = 2**np.linspace(np.log2(.01), np.log2(1), 20)
        ...
        for (kernel_func, desc), metric \
            ...
            for bandwidth_percentile in hyperparams:
                cindex_scores = []
                ...
                for train_idx, val_idx in kf.split(X_train):
                    ...
                    pairwise_dists \
                        = np.sort(pdist(fold_X_train_standardized).flatten())
                    if bandwidth_percentile < 1:
                        bandwidth = pairwise_dists[int(bandwidth_percentile
                                                       * len(pairwise_dists))]
                    else:
                        bandwidth = pairwise_dists[-1]  # max dist

                    surv_model = KernelSurvival(radius=bandwidth, n_jobs=-1,
                                                metric=metric)
                    ...
                    surv = \
                        surv_model.predict_proba(fold_X_val_standardized,
                                                 mesh_points,
            ...
            if arg_max_cindex is not None:
                ...
                print('Best bandwidth percentile for maximizing '
                      + 'training c-index: %5.4f -- achieves score %5.4f'
            ...
            for idx, (q, horizon) in enumerate(zip(IPEC_percentiles,
                ...
                if arg_min_IPEC_scores[idx] is not None:
                    ...
                    print('Best bandwidth percentile for minimizing training '
                          + 'IPEC (%.2f) score: %5.4f -- achieves score %5.4f'
                          % (q, arg_min_IPEC_scores[idx],
            ...
            for bandwidth_percentile in best_hyperparams:
                scaler = StandardScaler()
                X_train_standardized = scaler.fit_transform(X_train)
                X_test_standardized = scaler.transform(X_test)

                pairwise_dists = np.sort(pdist(X_train_standardized).flatten())
                if bandwidth_percentile < 1:
                    bandwidth = pairwise_dists[int(bandwidth_percentile
                                                   * len(pairwise_dists))]
                else:
                ...
                surv = \
                    surv_model.predict_proba(X_test_standardized, mesh_points,
                                             presorted_times=True,
                                             kernel_function=kernel_func)
                ...
                if 'cindex' in hyperparameter_to_score[bandwidth_percentile]:
                    final_test_scores['cindex'] = test_cindex
                for q, horizon in zip(IPEC_percentiles, IPEC_horizons):
                    if q in hyperparameter_to_score[bandwidth_percentile]:
                        final_test_scores[q] = \
